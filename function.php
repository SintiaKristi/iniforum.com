...
function add_new_member() {
	...
	if( !count( $err ) ) {
		if( num( query( "SELECT * FROM ".TABLE_USER." WHERE email='$email'" ) ) == 0 ) {
			query( "INSERT INTO ".TABLE_USER." VALUES( '$user_id', '$fullname', '".md5( $password )."', '$email', ' ', 'N', '".time()."', '".time()."', '0' )" );
...
# Fungsi untuk mengubah data topik
function update_topic( $id ) {
	...
	if( !count( $err ) ) {
		query( "UPDATE ".TABLE_TOPIC." SET date='".time()."', title='$title', descript='$descript' WHERE topic_id='$id'" );
	}
	if( count( $err ) ) { $_SESSION['msg-update-topic']['fail'] = implode( '<br>', $err ); }
	if( count( $err ) ) { header( "Location: ".SITE_URL."/topic.php?option=update-topic&topicid=".$_GET['cattopicid'] ); }
	else { header( "Location: ".SITE_URL."/topic.php?option=view-topic&topicid=".$_GET['topicid'] ); }
	exit;
}
...
function update_profile( $id ) {
	...
	//if( empty( $password ) ) { $err[] = '- Mohon untuk mengisi password Anda'; }
	if( empty( $email ) ) { $err[] = '- Isi alamat email Anda'; }
	//if( empty( $url ) ) { $err[] = '- Isi alamat website Anda'; }
	if( !count( $err ) ) {
		...
		if( in_array( $ext ,$valid_formats ) ) {
			...
			$row = fetch( query( "SELECT * FROM ".TABLE_USER." WHERE user_id='$id'" ) );
			$pass = empty( $password ) ? $row['password'] : md5( $password );
			query( "UPDATE ".TABLE_USER." SET fullname='$fullname', password='$pass', email='$email', url='$url', photo='$new' WHERE user_id='$id'" );
			if( move_uploaded_file( $_FILES['photo']['tmp_name'], $upload ) ) {
				
			} else {
				$err[] = '- Maaf, gambar profil gagal diupload';
			}
		} else {
			$err[] = '- Maaf, format gambar tidak diizinkan';
		}
	}
	if( count( $err ) ) { $_SESSION['update-profil']['gagal'] = implode( '<br>', $err ); }
	if( count( $err ) ) { header( "Location: ".SITE_URL."/profile.php?option=update-profile&userid=$id" ); }
	else { header( "Location: ".SITE_URL ); }
	exit;
}
...
function check_member_login() {
	...
	if( !count( $err ) ) {
		...
		else { $err[] = '- Maaf, email dan password Anda salah'; }
	}
...
function admin_login_form() {
	...
	echo "	Email:<br> <input type=\"text\" name=\"email\" size=\"26\"><br>";
	echo "	Password:<br> <input type=\"password\" name=\"password\" size=\"26\"><br>";
...
function member_login_form() {
	...
	echo "	<form method=\"post\" action=\"\" autocomplete=\"off\">";
	echo "	Email:<br> <input type=\"text\" name=\"email\" size=\"26\" id=\"email\" value=\"$email\"><br>";
	echo "	Password:<br> <input type=\"password\" name=\"password\" id=\"password\" size=\"26\"><br>";
...
function create_account_form() {
	...
	echo "	<form method=\"post\" action=\"\" autocomplete=\"off\" class=\"reg\">";
...
function update_topic_form( $id ) {
	$row = fetch( query( "SELECT * FROM ".TABLE_TOPIC." WHERE topic_id='$id'" ) );
	echo "<div class=\"box\">";
	echo "	<h1>Ubah Topik : ".$row['title']."</h1>";
	...
	if( isset( $_SESSION['user_id'] ) ) {
		...
		echo "	<form method=\"post\" action=\"\" autocomplete=\"off\">";
		echo "	Judul Topik Anda:<br> <input type=\"text\" name=\"title\" size=\"82\" id=\"title\" value=\"".$row['title']."\"><br>";
		echo "	Detail Topik Anda:<br> <textarea name=\"descript\" rows=\"10\" cols=\"87\">".$row['descript']."</textarea><br>";
		echo "	<input type=\"submit\" name=\"action\" value=\"Update Topic\" id=\"button\">";
		echo "	</form>";
		echo "	<script>document.getElementById('title').focus();</script>";
		echo "	<input type=\"hidden\" name=\"catid\" value=\"".$_GET['catid']."\">";
	} else {
...
function get_categories() {
	...
	echo "<table border=\"1\">";
	echo "<tr class=\"tops\">";
	echo "	<td width=\"23\" align=\"center\">NO</td>";
	echo "	<td width=\"480\">Judul Kategori</td>";
	echo "	<td width=\"160\">Oleh</td>";
	echo "	<td width=\"65\" align=\"center\">Tanggal</td>";
	...
	if( num( $sql ) == 0 ) { echo "<tr><td colspan=\"5\">Maaf, belum ada kategori untuk saat ini.</td></tr>"; }
		...
		$no = 1; $a = 0;
		while( $row = fetch( $sql ) ) {
			if( $a == 0 ) { $bg = "#292a2b"; $a = 1; }
			else { $bg = "#252627"; $a = 0; }
			$num = num( query( "SELECT * FROM ".TABLE_TOPIC." WHERE cat_id='{$row['cat_id']}'" ) );
			echo "<tr bgcolor=\"$bg\" onmouseover=\"bgColor='#2c2f31'\" onmouseout=\"bgColor='$bg'\">";
			echo "	<td valign=\"top\" align=\"center\"><span class=\"number\">{$no}</span></td>";
			echo "	<td><span style=\"text-transform: uppercase; font-weight: bold; \"><a href=\"".SITE_URL."/category.php?option=view-category&catid={$row['cat_id']}\">{$row['cat_name']}</a></span><br><span style=\"font-size: 11px; line-height: 1.3em;\">{$row['cat_desc']}</span></td>";
...
function view_category() {
	...
	echo "	</tr>";
	if( num( $sql ) == 0 ) { echo "<tr height=\"40\"><td colspan=\"6\" align=\"center\">Belum ada data topik saat ini. Jadilah yang pertama</td></tr>"; }
	else {
		$no = 1; $a = 0;
		while( $row = fetch( $sql ) ) {
			...
			echo "<tr bgcolor=\"$bg\" onmouseover=\"bgColor='#2c2f31'\" onmouseout=\"bgColor='$bg'\">";
			echo "	<td valign=\"top\" align=\"center\">{$no}<br>";
			if( $row['user_id'] == $_SESSION['user_id'] ) {
				echo "	<a href=\"".SITE_URL."/topic.php?option=update-topic&topicid={$row['topic_id']}\" class=\"update-topic\" title=\"Update this topic\">Edit</a>";
			}
			echo "	</td>";
			echo "	<td><span style=\"font-weight: bold;line-height:20px; \"><a href=\"".SITE_URL."/topic.php?option=view-topic&topicid={$row['topic_id']}\">{$row['title']}</a></span><br><span style=\"font-size: 11px; line-height: 1.3em;\">".substr( $row['descript'], 0, 100 )."...</span></td>";
			echo "	<td><b>{$row['fullname']}</b></td>";
			echo "	<td align=\"center\">".date( "d/m/Y", $row['add_date'] )."</td>";
			echo "	<td align=\"center\">$num</td>";
...
function view_topic() {
	...
	echo "	<p>Pengirim: <b>".$sql['fullname']."</b> | <a href=\"".SITE_URL."/category.php?option=view-category&catid=".$sql['cat_id']."\">".$sql['cat_name']."</a> | ".date( "F d, Y", $sql['add_date'] )." | Dibaca: <b>".$sql['views']."</b> kali ";
	if( $sql['user_id'] == $_SESSION['user_id'] ) {
		echo "	| <a href=\"".SITE_URL."/topic.php?option=update-topic&topicid={$sql['topic_id']}\" title=\"Update this topic\">Edit</a>";
	}
	echo "	</p>";
...
# get all users
function nine_top_user() {
	$sql = query( "SELECT * FROM ".TABLE_USER." ORDER BY user_id DESC LIMIT 0,9" );
	echo "<div class=\"box\">";
	echo "	<h1>Top Users</h1>";
	while( $row = fetch( $sql ) ) {
		if( $row['photo'] == 'N' ) {
			echo "<a href=\"".SITE_URL."/view.php?userid={$row['user_id']}\" title=\"{$row['fullname']}\"><img src=\"".SITE_URL."/photo/who.jpg\" class=\"photo-small\"></a>";
		} else {
			echo "<a href=\"".SITE_URL."/view.php?userid={$row['user_id']}\" title=\"{$row['fullname']}\"><img src=\"".SITE_URL."/photo/".$row['photo']."\" class=\"photo-small\"></a>";
		}
	}
	echo "</div>";
}
...
function get_all_online_user() {
	$sql = query( "SELECT * FROM ".TABLE_USER." WHERE status='1'" );
	echo "<div class=\"box\">";
	echo "	<h1>Online</h1>";
	if( num( $sql ) == 0 ) {
		echo "<p>No one online...</p>";
	} else {
		while( $row = fetch( $sql ) ) {
			if( $row['photo'] == 'N' ) {
				echo "<a href=\"".SITE_URL."/view.php?option=view-user&userid={$row['user_id']}\" title=\"{$row['fullname']}\"><img src=\"".SITE_URL."/photo/who.jpg\" class=\"photo-small\"></a>";
			} else {
				echo "<a href=\"".SITE_URL."/view.php?option=view-user&userid={$row['user_id']}\" title=\"{$row['fullname']}\"><img src=\"".SITE_URL."/photo/".$row['photo']."\" class=\"photo-small\"></a>";
			}
		}
	}
	echo "</div>";
